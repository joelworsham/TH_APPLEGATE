...
add_action( 'widgets_init', function () {
	register_widget( 'Applegate_Widget_TextIcon' );
} );

/**
 * Class Applegate_Widget_TextIcon
 *
 * Widget for showing Text boxes with an Icon in the title.
 *
 * @since 1.0.0
 */
class Applegate_Widget_TextIcon extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['title'] ) ) {
			...
			if ( ! empty( $instance['icon'] ) ) {
				?>
				<span class="applegate-widget-icon icon-effect-small-light">
		...
		if ( ! empty( $instance['text'] ) ) {
			?>
			<div class="applegate-widget-text">
